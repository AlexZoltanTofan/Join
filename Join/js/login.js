setURL('https://alex-zoltan-tofan.developerakademie.net/smallest_backend_ever');
let userDates = [];
  ...
  if (user.Password == pw.value && !pw.value == '') {
    ...
    if (document.querySelector('#remember').checked) {
      ...
      localStorage.setItem(
        'localUserDates',
        JSON.stringify(setLocalUserDates(mail, pw))
      );
    ...
    !pw.value == '' &&
    !user.Password.includes(pw.value)
  ) {
    document.querySelector('.LoginNote').innerHtml = '';
    document.querySelector('.LoginNote').innerHTML =
      'The email or password you entered is incorrect.';
...
function setForgotPwEvent() {
  document
    .getElementById('senResetPwMailButton')
    .addEventListener('click', (event) => {
      event.preventDefault();
      loadTemplate(3);
    });
...
function loadChangePasswordEvent() {
  document.querySelector('button').addEventListener('click', (event) => {
    ...
    event.preventDefault();
    document
      .querySelector('.alertChangePW')
      .classList.add('alertChangePWloaded');
    setTimeout(() => {
      location.reload();
    }, 500);
  });
